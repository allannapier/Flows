...
export function RunParamsForm({
  ...
  if (params.length === 0) {
    return (
      <box flexDirection="column" flexGrow={1} backgroundColor={colors.bg} padding={2}>
        <text fg={colors.dim}>Starting "{flow.name}"...</text>
      </box>
    );
  }
    ...
    <box flexDirection="column" flexGrow={1} backgroundColor={colors.bg}>
      ...
      >
        {params.map((p, i) => {
          ...
          return (
            <box key={p.name} flexDirection="column">
              <text fg={focused ? colors.accent : colors.text}>
                {p.name}
                {p.required ? " *" : ""}
        ...
        <box
          border
          borderStyle="rounded"
          borderColor={focusIndex === params.length ? colors.accent : colors.dim}
          padding={1}
        >
          <text fg={focusIndex === params.length ? colors.accent : colors.text}>▶ Start flow</text>
        </box>
        {error && <text fg={colors.error}>{error}</text>}
      </box>
      <Hint>enter next/start · up/down move · esc back</Hint>
    </box>
  );
}
